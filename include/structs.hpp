...
// struct <название> (Plain data object)
struct Student {
...
struct University {
 public:  // публичный спецификатор доступа - поля и методы являются открытыми для использования
  ...
  University() : University("", 0) /* делегирующий конструктор */ {
    ...
    this->pointer_ = new int{1};  // выделение памяти в куче (нужно будет высвободить память)
  ...
  // конструктор от 2 параметров (move semantics)
 ...
 public: // публичные поля
  ...
  static int ID;  // статическое поле - общее для всех объектов структуры
  // если не инициализировать ID, то он будет хранить "мусор"
 ...
 private: // приватный спецификатор доступа - поля и методы являются закрытыми для использования вне структуры
  ...
  int id_;
  std::string name_;
  int ranking_{-1};  // инициализация вне конструктора
...
int University::CurrentID() {
  ...
  return University::ID;  // можно просто return ID
}
