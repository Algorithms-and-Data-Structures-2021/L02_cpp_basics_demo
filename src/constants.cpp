...
int main() {
  ...
  {  // константа времени выполнения (runtime) - const
    ...
    // Тема для самостоятельного изучения (optional): r-value и l-value
    ...
    // const_ptr = nullptr;  // <- ошибка компиляции

    // изменение значения переменной - OK
    *const_ptr = 3;
  }
  ...
  {  // константность времени компиляции (compile time) - constexpr
    ...
    const int res3 = constexpr_function(var);  // выполнится в runtime
